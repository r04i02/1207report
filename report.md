...
- 説明
   - 敵はスクショ上から2番目のもののように描画される。
   - 移動速度に関しては以下のように設定されている
     - x軸方向に関するもの
       - 元の加速度を上昇させる(vx += ax;)
       - 加算されたものに経過時間の2倍にphaseを加算する、これを仮にtmpとする。そのとき、移動する速度は$vx*sin(tmp)dt$と表される これ自体が速度となる
      - y軸方向に関するもの
        - x軸においてtmpだったものはmtが2倍されていたが、y軸においては5倍されている。
        - x軸においてはsinだったが、y軸においてはcosとなっている。
...
- 説明
  - scoreが多重加算されないようにshootingApp.cppを改変した。(コードは以下に)
```cpp
if (enemies[i]->checkCollision(missiles[j])) {
	score.addScore(enemies[i]->score);
}
```
